...
- Added support for the [Bun](https://bun.sh/) runtime when compiling to
  JavaScript by using `rakun run --target javascript --runtime bun`
- Allow compilation of packages that require `"rebar"` using the rebar3
  compiler.
- A warning is now emitted if there is a `.rakun` file with a path that would be
  invalid as a module name.
- The `~> x.y` version constraint syntax has been dropped in favour of
  `> x.y.z and <= xx.0.0` syntax in `rakun add` and `rakun new`, for clarity.
...
- Fixed a bug where the compiler would panic when it cannot get current
  directory.
- Improved error message for non-UTF-8 paths encountered during Rakun commands.
...
- Fixed a bug where on windows local packages had absolute paths in the manifest
  instead of relative.
- The `rakun publish` command now asks for confirmation if the package
  repository URL doesn't return a successful status code.
- `rakun publish` can now optionally use a Hex API key to authorise publishing
  and retiring packages, set via the environment variable `HEXPM_API_KEY`.
- `rakun publish` will now refuse to publish placeholder packages, to prevent
...
- The `Compiling Rakun` message is no longer emitted each time code is compiled.
...
- Fixed a bug where hovering over an expression in the middle of a pipe would
  give the wrong node.
- Go to definition now works for values in dependency Rakun modules.
...
- `rakun export` now takes a `package-interface` option to export a json file
  containing metadata about the root package.
- `rakun docs build` now creates a json file containing metadata about the root
  package.
...
- It is now possible to use `rakun run -m` to run a dependency module even if
...
- A warning is no longer emitted when a function has a Rakun implementation as
  well as external implementations for both targets. This is because having a
  default Rakun implementation means the code is future-proof and continues to
...
- Fixed a bug where `rakun run` would fail when the current directory is not
...
- The `rakun new` command now accepts any existing path, as long as there are
  no conflicts with already existing files. Examples: `rakun new .`, `rakun new
..`, `rakun new ~/projects/test`.
- The format for the README created by `rakun new` has been altered.
- The `rakun.toml` created by `rakun new` now has a link to the full reference
  for its available options.
- The `rakun` binary is now statically linked on Windows.
...
- `rakun` commands now look in parent directories for a `rakun.toml` file.

### Bug fixes

- Fixed a bug where `rakun add` would not update `manifest.toml` correctly.
...
- Fixed a bug where `rakun new` would generate a gitignore with `build` rather
  than `/build`.
...
- Fixed a bug where `rakun deps list` would look in filesystem root instead of
...
- Fixed a bug with the `isEqual` function in `prelude.js` where RegExps were
  ...
  equal.
- JavaScript: export from `prelude.d.mts` in `rakun.d.mts` to fix the error:
...
- `rakun format` now ignores the Rakun build directory by default, even when not
  in a git repository.

### Language changes

- Rakun now has full exhaustiveness checking. Exhaustiveness issues have been
  downgraded from errors to warnings so that existing Rakun code can be
...
- The `rakun run` and `rakun test` commands gain the `-t` flag, which is an
  alias of the `--target` flag.
- The `rakun build`, `rakun check`, `rakun run` and `rakun test` commands now
  also accept `js` and `erl` as values for the `--target` flag.
- The `rakun new` command now creates packages at version 1.0.0.
- The `rakun publish` command now asks for confirmation if the package being
  published is not yet version 1.0.0.
- The `rakun publish` command now asks for confirmation if the package name is
  one that implies the package is maintained by the Rakun core team.
...
- Fixed the `target` property of `rakun.toml` being ignored for local path
  dependencies by `rakun run -m module/name`
...
- New projects are now generated with the call to `rakun format` coming last in
  the GitHub Actions workflow. This is so that feedback from tests is presented
  even if formatting is incorrect.
- Added Windows support for the `rakun export erlang-shipment` command.
...
- New Rakun projects are created with `rakun_stdlib` v0.32 and `gleeunit` v1.0.

### Bug fixes

- Fixed a bug where `rakun fix` would not produce correct results for code that
...
- Fixed a bug where running `rakun fix` multiple times could produce incorrect
  results.
...
- Fixed a bug where `rakun fix` would fail to update the deprecated type import
...
- Using `import module.{TypeName}` to import a type has been deprecated,
  replaced by `import module.{type TypeName}`. In a future version of Rakun the
  old syntax will only import the value of the same name. Run `rakun fix` to
  update your code.
- The `BitString` type has been renamed to `BitArray`. Run `rakun fix` to update
  your code.
...
- The error message for when one tries to access an unknown field has been
  improved.
- The `__rakun_prelude_variant__` property has been removed from the classes
...
- The `rakun check` command supports the `target` flag.
- The `rakun fix` command updates code to use `BitArray` rather than `BitString`.
- The `rakun fix` command updates code to use the new import type syntax.
- `rakun fix` sets the `rakun` version constraint in `rakun.toml` to `>= 0.32.0`.
- The `rakun` version constraint field in `rakun.toml` now disregards pre and
...
- The prelude is no longer rendered once per package when compiling to
  ...
  using the `rakun compile-package` API you now need to give a path to the
  prelude using the `--javascript-prelude` flag.
- The `rakun export javascript-prelude` and `rakun export typescript-prelude`
  commands have been added to export a copy of the prelude. This command may be
  useful for build tools that use the compiler via the `rakun compile-package`
  API.
...
- "Compiling $package" is now only printed when a package has new changes to
  compile.
- The main process started with `rakun run` no longer traps exits on Erlang.
...
- New Rakun projects are created with `rakun_stdlib` v0.31, `actions/checkout`
...
- The `if` conditional compilation, `external fn`, and `external type` syntaxes
  have been removed.
- The `description` flag for the `rakun new` command has been removed.
...
- New Rakun projects are created with `rakun_stdlib` v0.30, Erlang OTP v26.0.2,
...
- Fixed a bug where `rakun fix` would fail to parse command line flags.

## v0.30.0-rc2 - 2023-07-03

- Fixed a bug where `rakun fix` would merge external functions of the same name
...
- The new `@target(erlang)` and `@target(javascript)` attribute syntax has been
  ...
  syntax has been deprecated. Run `rakun fix` to update your code.
- The new `type TypeName` syntax syntax replaces the `external type TypeName`
  syntax. The existing external type syntax has been deprecated. Run `rakun format`
  to update your code.
...
- Dependencies can now be loaded from paths using the
  `packagename = { path = "..." }` syntax in `rakun.toml`.
- The `javascript.deno.unstable` field in `rakun.toml` can now be used to
  enable Deno's unstable APIs when targeting JavaScript.
- Blockquotes are now styled in rendered HTML documentation.
- The `rakun` property can be set in `rakun.toml` can be set to a version
  requirement to specify the version of Rakun required to build the project.
...
- Fixed a bug where `rakun docs build` would print an incorrect path upon
  completion.
- Warnings from dependency packages are no longer surfaced in the language
  server.
- A warning is now emitted when a Rakun file is found with an invalid name.
...
- The new `rakun remove <package_name>` can be used to remove dependencies
  from a Rakun project.
- Fixed a bug where the formatter could crash.
- Fixed a bug where invalid Erlang would be generated when piping into `panic`.
- The `rakun docs build` command gains the `--open` flag to open the docs after
...
- New projects now require `rakun_stdlib` v0.29.

## v0.29.0-rc2 - 2023-05-22

- The `rakun lsp` command is no longer hidden from the help output.
...
- Fixed a bug where the language server would not reset the build directory when
  it was created by a different version of Rakun.
- New Rakun projects are created with `erlef/setup-beam@v1.15.4` in their GitHub
  actions CI configuration.
- Running a module now uses the dependency's target and runtime in its `rakun.toml`.
...
- Fixed a bug where the language server would attempt to analyse Rakun modules
  that were outside of the `src` or `test` directories.
- New Rakun projects are created with `actions/checkout@v3.5.1` and
...
- New projects now require `rakun_stdlib` v0.28.
...
- Warnings are now immediately emitted rather than being buffered until the end
  of the compilation.
- The `--warnings-as-errors` flag is now supported by `rakun build`.
...
- Allow modules to be run with the `rakun run --module` command.

## v0.27.0 - 2023-03-01

- Fixed a bug where `panic` could generate incorrect JavaScript code.
- New projects now require `rakun_stdlib` v0.27.
...
- The new `rakun fix` command can be used to automatically convert `try`
...
- `let assert ... = ...` is now the syntax for assertion assignments. The
  ...
  version. Run `rakun format` to automatically update your code.
- `rakun export hex-tarball` can be used to create a tarball suitable for
...
[Release blog post](https://rakun.run/news/v0.26-incremental-compilation-and-deno/)

- New projects require `rakun_stdlib` v0.26 and `gleeunit` v0.9.
...
- Running `rakun new` will skip `git init` if the new project directory is
...
- New Rakun projects are created using GitHub actions erlef/setup-beam@v1.15.2.
- Some modules can now be hidden from the docs by specifying a list of glob
  patterns in `internal_modules` in `rakun.toml`. The default value for this
  list is `["$package_name/internal", "$package_name/internal/*"]`.
- The `rakun new` command gains the `--skip-git` flag to skip creation of
  `.git/*`, `.gitignore` and `.github/*` files.
- The `rakun new` command gains the `--skip-github` flag to skip creation of
  `.github/*` files.
...
- Updated `actions/checkout` from `actions/checkout@v3.0.0` to `@v3.2.0` for
  projects created via `rakun new`.
- Fixed a bug where `rakun new` would set a `Rebar3` version to `25.1`
  instead of the latest stable `3`.
- Updated following runtime versions set via `rakun new`: `Erlang/OTP`
...
- New Rakun projects are now configured to explicitly install rebar3 using
...
- New Rakun projects are created using GitHub actions erlef/setup-beam@v1.15.0.
...
[Release blog post](https://rakun.run/news/v0.25-introducing-use-expressions/)

## v0.25.0-rc2 - 2022-11-23

- Fixed a bug where Rakun dependency packages with a `priv` directory could fail
  to build.
...
- Nightly builds are now also published as OCI container images hosted on
  GitHub.
- Fixed a bug where the build tool would not hook up stdin for Rakun programs it
  starts.
...
- New Rakun projects are created using GitHub actions erlef/setup-beam@v1.14.0
- The `javascript.typescript_declarations` field in `rakun.toml` now applies to
  the entire project rather than just the top level package.
- The formatter now adds a `0` to floats ending with `.` (ie `1.` => `1.0`).
- New projects require `rakun_stdlib` v0.25.

## 0.24.0 - 2022-10-25

[Release blog post](https://rakun.run/news/rakun-v0.24-released/)
...
- Rakun can now compile Elixir files within a project's `src` directory.
...
- New projects depend on `rakun_stdlib` v0.24.
...
- If you attempt to use the method call syntax (`thing.method()`) on a value
  ...
  Rakun is not object oriented and does not have methods.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.23-released/)

## v0.23.0-rc2 - 2022-09-15

- New Rakun projects are created using GitHub actions erlef/setup-beam@v1.13.0
  and actions/checkout@v3.0.0.
- New Rakun projects are created using version v0.23.0 of the stdlib.
- Fixed a bug where LSP hovering would fail to locate the expression.

## v0.23.0-rc1 - 2022-09-01

- Rakun can now build dependency packages that are managed using Mix.
- Compiler performance has been improved by buffering disc writing and by lazily
  ...
  standard library.
- The `rakun publish` command now adds the `priv` directory and any `NOTICE`
  file to the tarball.
- The `rakun update` command can now be used to update dependency packages to
...
- In JavaScript, if an object has defined an `equals` method in its prototype,
  Rakun will now use this method when checking for equality.
...
- Fixed a bug where `rakun new` would in some situations not validate the
...
- The `rakun publish` confirmation prompt now accepts both "Y" and "y".
- Fixed a bug where `todo` would not emit the correct line number to the LSP while.

## v0.22.0 - 2022-06-12

[Release Blog Post](https://rakun.run/news/rakun-v0.22-released/)

- New projects are created with `rakun_stdlib` v0.22.
...
- Added the ability to replace a release up to one hour after it is published
  using `rakun publish --replace`.
- `rakun publish`, `rakun docs publish`, `rakun docs remove`, `rakun hex retire`,
  and `rakun hex unretire` now have access to environment variables for
  username (default key `HEXPM_USER`) and password (default key `HEXPM_PASS`)
- The `rakun publish` command gains the `-y/--yes` flag to disable the "are you
  sure" prompt.
...
- The compiler is now able to generate TypeScript declaration files on target
  JavaScript (#1563). To enable this edit `rakun.toml` like so:
...
- The project generator will no longer permit use of the reserved `rakun_`
  prefix.
- Generated HTML docs easter egg updated.
- `rakun export erlang-shipment` can be used to create a directory of compiled
  Erlang bytecode that can be used as a deployment artefact to get your
  application live.
- `rakun format` will now preserve (up to one) empty lines between consecutive
  comments, as well as between comments and any following expression
- The deprecated rebar3 integration has been removed.
- Fixed a bug where `rakun format` would output an unwanted newline at the top
...
- Fixed a bug where `rakun format` would remove the braces around a tuple index
...
- The `rakun compile-package` command no longer generates a `.app` file. This
...
[Release Blog Post](https://rakun.run/news/v0.21-introducing-the-rakun-language-server/)

- New projects are created with `rakun_stdlib` v0.21.
...
- The language server will now enter a degraded mode that only performs
  formatting if running in a directory that is not a Rakun project with a
  `rakun.toml`.

## v0.21.0-rc1 - 2022-04-16

- The Rakun language server is here! This will provide IDE like features for
...
- Log messages controlled with `RAKUN_LOG` now print to standard error.
- Log message colours can be disabled by setting the `RAKUN_LOG_NOCOLOUR`
  environment variable.
- You can now specify multiple packages when using `rakun add`.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.20-released/)

- New projects are created with `rakun_stdlib` v0.20.
...
- Built in support for the Mix build tool has been removed. The `mix_rakun`
...
- The format used by the formatter has been improved.
- Package names in `rakun.toml` are validated when the config is read.
- The `priv` directory is linked into the build directory for Rakun projects
...
[Release Blog Post](https://rakun.run/news/rakun-v0.19-released/)

## v0.19.0-rc4 - 2022-01-10

- New projects are created with `rakun_stdlib` v0.19 and `gleeunit` v0.6.
...
- New projects are created with `erlef/setup-beam` v1.9.0 instead of
  `rakun-lang/setup-erlang` and `rakun-lang/setup-rakun`.
...
- Fixed a bug where `include` directories would not be accessible by the Erlang
  compiler during Rakun compilation.

## v0.19.0-rc1 - 2022-01-03

- The build tool now supports the JavaScript target. The target can be specified
  in either `rakun.toml` or using the `--target` flag.
- The `rakun check` command has been introduced for rapidly verifying the types
  of Rakun code without performing codegen.
- `true` and `false` can no longer be used as pattern matching variables, to
  ...
  An error will hint about using Rakun's `True` and `False` values instead.
- You can now remove build artifacts using the new `rakun clean` command.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.18-released/)

- New projects now include `gleeunit`.

## v0.18.0-rc3 - 2021-12-05

- URL format in rakun.toml is now validated.
- The `rakun deps list` command has been added.
- Fixed a bug where changing requirements in `rakun.toml` would not cause deps
  to be re-resolved.
...
- `rakun.toml` now includes an `erlang.extra_applications` key to specify extra
...
- Rakun can now compile Rakun projects.
- Rakun can now run tests with the `rakun eunit` command.
- Rakun can now run programs with the `rakun run` command.
- Rakun can now run an Erlang shell with the `rakun shell` command.
- Rakun can now resolve package versions for a Rakun project's dependency tree.
- Rakun can now download Hex packages.
- Rakun can now build dependency packages that are managed using Rakun or
  rebar3.
- Rakun is now the default build tool for new projects.
- The template names for `rakun new` have been changed.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.17-released/)
...
- New projects are generated using `rakun_stdlib` v0.17.1.

## v0.17.0-rc1 - 2021-09-11

- Redesigned the Rakun prelude to be a module of core classes when compiling to
  JavaScript. This improves the resulting generated code and makes debugging and
  interop easier.
- Projects without rebar3 can be generated using the `rakun-lib` template.
...
- Support for the deprecated `tuple(x, y, ...)` syntax has been removed in favor
  of the more concise (`#(x, y, ...)`). Use `rakun format` with the previous
  version of the compiler to auto-migrate.
- New OTP projects are generated using `rakun_otp` v0.1.6.
...
- Rakun modules with no public exports no longer render private members in
  Erlang.
...
- Rakun packages imported in JavaScript now have the path prefix
  `rakun-packages`. This can be served from your web server or aliased in your
...
- New projects are generated using `rakun_stdlib` v0.17.0.
- New projects are generated using `rakun_otp` v0.2.0.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.16-released/)
...
- New projects are generated using `rakun_stdlib` v0.16.0.

## v0.16.0-rc2 - 2021-06-08

- Rakun now supports alternative patterns in case expressions for the JavaScript target.
- The `rakun` prelude module can now be imported when compiling to JavaScript.
...
- Rakun can now compile to JavaScript! Specify the `--target javascript` flag to
  `rakun compile-package` to use it today.
...
- The `rakun new` command requires a root folder param, project name is
...
- New OTP application projects depend on `rakun_otp` v0.1.5.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.15-released/)

## v0.15.0-rc1 - 2021-05-05

- Syntax highlighting of Rakun code in generated HTML documentation has been
  improved.
...
- Glean `new` command now checks if target folder exists, if so it returns
  an error.
- A compile time error is now raised if a module is defined with the name `rakun`.
- A compile time error is now raised if a module is defined with the a keyword
  in the name.
- New projects are generated using `rakun_stdlib` v0.15.0.
- New projects are generated at v0.1.0.

## v0.14.4 - 2021-03-27

- The Rakun compiler has been updated to compile with the new Rust v1.51.0.
- New project's `rakun.toml` has a comment that shows how to add a
  `repository` field.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.14-released/)

## v0.14.0-rc2 - 2021-02-18

- New projects are created with `rakun_stdlib` v0.14.0.

## v0.14.0-rc1 - 2021-02-14

- Rakun now generates Erlang typespecs.
...
- `--warnings-as-errors` flag added to `rakun build` command.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.13-released/)

- New Rakun projects use stdlib v0.13.0.

## v0.13.0-rc2 - 2021-01-12

- The `version` property in `rakun.toml` is now optional again.
...
[Release Blog Post](https://rakun.run/news/rakun-v0.12-and-rakun-otp-v0.1-released/)
...
- New application projects are now created used Rakun's type safe OTP pulled
  from Hex.
...
- New projects are created with version 1.0.2 of the setup-rakun GitHub
  action.
- New application projects are now created used Rakun's type safe OTP.
...
[Release Blog Post](https://lpil.uk/blog/rakun-v0.11-released/)
...
- The formatter style has been improved to render some single argument calls
  in a more compact style.
- Rakun now supports hex, octal, and binary literals.
- Rebar3 hex packages now include `rakun.toml` and `gen`.
...
[Release Blog Post](https://lpil.uk/blog/rakun-v0.10-released/)
...
- Fix code generation when calling functions returned through either record
  or tuple access
- Add lookup for Rakun source code in Mix's `deps` directory.
- Newly generated Rakun projects use the GitHub action
  `rakun-lang/setup-erlang` v1.1.0.
...
[Release Blog Post](https://lpil.uk/blog/rakun-v0.8-released/)
...
- HTML documentation can now be generated from Rakun code by running `rakun build --doc`.
- Rakun code can be formatted using the `rakun format` command.
...
- Projects generated with `rakun new` use `stdlib` version 0.7.0.

## v0.7.0 - 2020-03-01

[Release Blog Post](https://lpil.uk/blog/rakun-v0.7-released/)
...
- `rakun new` now creates a project that uses `actions/checkout@v2.0.0` in its
...
- A friendly error message is rendered when a .rakun file cannot be read.
- A friendly error message is rendered when the `rakun new` command fails to
...
- `rakun new` now includes installation instructions for Hex packages in the
  generated README.
- `rakun new` now accepts a `--description` flag for including a description of
...
[Release Blog Post](https://lpil.uk/blog/rakun-v0.6-released/)
...
[Release Blog Post](https://lpil.uk/blog/rakun-v0.5-released/)

- Enum constructor arguments can now be labelled, allowing arguments to be
  given by name at the call site.
- An Erlang header file with a record definition is generated for each Rakun
  struct defined.
- `rakun new` creates a project at v1.0.0.
...
- Anonymous structs have been introduced, serving as a quick and generic
  ...
  tuples.
- `rakun new` now accepts a `--template` flag to generate different styles of
  project. An OTP application template has been added alongside the existing
  OTP library template.
- `rakun new` now creates configuration for GitHub Actions, making Rakun
...
[Release Blog Post](https://lpil.uk/blog/rakun-v0.4-released/)
...
[Release Blog Post](https://lpil.uk/blog/rakun-v0.3-released/)

- New project structure can be generated with the `rakun new` command.
...
- Type error arising from the arguments of function calls have more accurate
  error diagnostics.
- Precompiled Rakun binaries are now available on the GitHub release page.
- Precompiled Docker images containing the Rakun binary are now available on
  DockerHub.
...
[Release Blog Post](https://lpil.uk/blog/hello-rakun/)
