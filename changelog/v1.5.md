...
- The `rakun fix` command can now update a project's `rakun` version contraint
  to make sure it respects the inferred minimum required version.
  ([Giacomo Cavalieri](https://github.com/giacomocavalieri))

- The build tool now refuses to publish a project where the `rakun` version
  constraint would include a compiler version that doesn't support the features
  used by the package.
  ([Giacomo Cavalieri](https://github.com/giacomocavalieri))

- If a project doesn't specify a `rakun` version constraint, the build tool will
  automatically infer it and add it to the project's `rakun.toml` before
  publishing it.
  ([Giacomo Cavalieri](https://github.com/giacomocavalieri))
...
- The compiler can now infer the minimum Rakun version needed for your code to
  compile and emits a warning if the project's `rakun` version constraint
  doesn't include it.
  For example, let's say your `rakun.toml` has the constraint
  `rakun = ">= 1.1.0"` and your code is using some feature introduced in a later
  version:
  ...
  warning: Incompatible rakun version range
    ┌─ /Users/giacomocavalieri/Desktop/datalog/src/datalog.rakun:1:22
    │
