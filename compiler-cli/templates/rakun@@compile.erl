    ...
    case file:read_line(standard_io) of
        ...
        {ok, Line} ->
            ...
            case compile_package(Lib, Out, Modules) of
                ok -> io:put_chars("rakun-compile-result-ok\n");
                err -> io:put_chars("rakun-compile-result-error\n")
            end,
            compile_package_loop()
    end.
...
configure_logging() ->
    Enabled = os:getenv("RAKUN_LOG") /= false,
    persistent_term:put(rakun_logging_enabled, Enabled).

log(Term) ->
    case persistent_term:get(rakun_logging_enabled) of
