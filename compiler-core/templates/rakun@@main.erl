...
refine_first(#{rakun_error := _, line := L}, [{M, F, A, [{file, Fi} | _]} | S]) ->
    [{M, F, A, [{file, Fi}, {line, L}]} | S];
refine_first(_, S) ->
    S.

error_class(_, #{rakun_error := panic}) -> "panic";
error_class(_, #{rakun_error := todo}) -> "todo";
error_class(_, #{rakun_error := let_assert}) -> "let assert";
error_class(Class, _) -> ["Erlang ", atom_to_binary(Class)].

error_message(#{rakun_error := _, message := M}) ->
    M;
error_message(undef) ->
...
error_message(_) ->
    <<"An error occurred outside of Rakun."/utf8>>.

error_details(_, #{rakun_error := let_assert, value := V}) ->
    ["unmatched value:\n  ", print_term(V), $\n, $\n];
error_details(_, {case_clause, V}) ->
    ["unmatched value:\n  ", print_term(V), $\n, $\n];
error_details(_, {badmatch, V}) ->
    ["unmatched value:\n  ", print_term(V), $\n, $\n];
error_details(_, #{rakun_error := _}) ->
    [];
