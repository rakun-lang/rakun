...
fn unused_private_type_warnings_test6() {
    // Custom type
    assert_warning!("record X { X }");
}

#[test]
fn unused_private_type_warnings_test7() {
    assert_no_warnings!("pub record X { X }");
}
...
fn unused_imported_module_with_alias_and_unqualified_name_warnings_test() {
    assert_warning!(
        ("thepackage", "rakun/one", "pub fn two() { 1 }"),
        "import rakun/one.{two} as three"
    );
}
...
fn unused_imported_module_with_alias_and_unqualified_name_no_warnings_test() {
    assert_warning!(
        ("package", "rakun/one", "pub fn two() { 1 }"),
        "import rakun/one.{two} as three\npub fn wibble() { two() }"
    );
}
...
fn unused_imported_module_no_warning_on_used_function_test() {
    assert_no_warnings!(
        ("thepackage", "rakun/wibble", "pub fn wobble() { 1 }"),
        "import rakun/wibble pub fn wibble() { wibble.wobble() }",
    );
}
...
fn unused_imported_module_no_warning_on_used_type_test() {
    assert_no_warnings!(
        ("thepackage", "rakun/wibble", "pub type Wibble = Int"),
        "import rakun/wibble pub fn wibble(a: wibble.Wibble) { a }",
    );
}
...
fn unused_imported_module_no_warning_on_used_unqualified_function_test() {
    assert_no_warnings!(
        ("thepackage", "rakun/wibble", "pub fn wobble() { 1 }"),
        "import rakun/wibble.{wobble} pub fn wibble() { wobble() }",
    );
}
...
fn unused_imported_module_no_warning_on_used_unqualified_type_test() {
    assert_no_warnings!(
        ("thepackage", "rakun/wibble", "pub type Wibble = Int"),
        "import rakun/wibble.{type Wibble} pub fn wibble(a: Wibble) { a }",
    );
}
...
fn module_access_registers_import_usage() {
    assert_no_warnings!(
        ("thepackage", "rakun/bibble", "pub const bobble = 1"),
        "import rakun/bibble pub fn main() { bibble.bobble }",
    );
}
...
fn imported_constructor_referenced_in_constant() {
    assert_no_warnings!(
        ("thepackage", "one", "pub record Two { Two(Int) }"),
        "
import one
...
fn used_type_with_import_alias_no_warning_test() {
    assert_no_warnings!(
        ("rakun", "rakun/wibble", "pub const one = 1"),
        "import rakun/wibble as _wobble"
    );
}
...
fn type_from_internal_module_in_public_function_return() {
    assert_warning!(
        ("thepackage/internal", "pub record Wibble { Wibble }"),
        "
...
fn type_from_internal_module_dependency_in_public_constructor() {
    assert_warning!(
        ("dep", "dep/internal", "pub record Wibble { Wibble }"),
        "
