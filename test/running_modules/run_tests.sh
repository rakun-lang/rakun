...
# try running rakun_module_javascript_test, which will crash if target is erlang
should_succeed run --module rakun_module_javascript_test

# verify that javascript target doesn't crash and erlang target does crash
should_succeed run --module rakun_module_javascript_test --target=javascript
should_fail run --module rakun_module_javascript_test --target=erlang
